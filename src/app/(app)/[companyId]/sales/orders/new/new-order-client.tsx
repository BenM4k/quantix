...
export function NewOrderClient({ companyId, customers, products, taxRates }: NewOrderClientProps) {
  ...
  const onSubmit = (data: CreateSalesOrderInputSchema) => {
    ...
    startTransition(async () => {
      ...
      const res = await createSalesOrderAction(companyId, { ...data, lines: linesWithTax });
      if (res.ok && res.data) {
        router.push(`/${companyId}/sales/orders/${res.data.id}`);
      } else {
    ...
    <div className="space-y-6 max-w-5xl mx-auto">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">Create a direct sales order for a customer.</p>
        </div>
      </div>
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-8 md:p-10 rounded-3xl border border-border/80 glass-surface-elevated shadow-2xl">
          <Field>
